import { LaneManager } from '../lane-manager.service';
...
describe('LaneManager Service', () => {
  ...
  beforeEach(() => {
    laneManager = new LaneManager(12, 30, 4000);
  ...
  describe('assignLane', () => {
    it('should assign first available lane', () => {
      ...
      expect(result.y).toBe(50); // 50 + (0 * 30)
    });
    ...
    it('should calculate Y position correctly', () => {
      ...
      expect(result1.y).toBe(50); // 50 + (0 * 30)
      expect(result2.y).toBe(80); // 50 + (1 * 30)
      expect(result3.y).toBe(110); // 50 + (2 * 30)
    });
    ...
    it('should reuse lanes after they expire', () => {
      ...
      // Move time forward past the duration
      const futureTime = mockTime + 5000;
      const result2 = laneManager.assignLane(futureTime);
      expect(result2.lane).toBe(0); // Lane 0 should be available again
    });
    ...
    it('should handle all lanes being occupied', () => {
      ...
      // Next assignment should reuse a lane
    ...
    it('should assign lane that will be free soonest when all occupied', () => {
      // Fill all lanes with different expiry times
      for (let i = 0; i < 12; i++) {
        laneManager.assignLane(mockTime + i * 100);
      }

      // The next assignment should go to lane 0 (expires soonest)
  ...
  describe('getLaneStatus', () => {
    it('should return correct occupation status', () => {
      ...
      expect(status.get(0)).toBe(true); // Occupied
      expect(status.get(1)).toBe(true); // Occupied
      expect(status.get(2)).toBe(false); // Free
  ...
  describe('edge cases', () => {
    it('should handle custom lane configuration', () => {
      const customManager = new LaneManager(5, 40, 2000);
      ...
      expect(result.y).toBe(50); // First lane
    });
